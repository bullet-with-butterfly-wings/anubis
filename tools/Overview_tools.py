...
def noise_time(alignment_mets, time = None):
    for idx, alg in enumerate(alignment_mets):
...
def cluster_size(chunks, seperate = False):
    ...
    for processedEvents, event_chunk in enumerate(chunks[1:]):
            ...
            for evt in clusters:
                for rpc in range(6):
                    for cluster in clusters[rpc]:
                        phi_histograms[rpc].append(cluster.size[0])
                        eta_histograms[rpc].append(cluster.size[1])
